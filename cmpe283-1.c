...
struct capability_info procbasedctl[22] =
{
...
struct capability_info procbasedctl2[27] =
{
...
struct capability_info exitctl[16] =
{
	...
	{ 26, "CLEAR_IA32_LBR_CTL"},
	{ 28, "LOAD_CET_STATE"},
	{ 29, "LOAD_PKRS"},
	{ 31, "ACTIVATE_SECONDARY_CONTROL"}
};


struct capability_info entryctl[13] =
{
	...
	{ 20, "LOAD_CET_STATE"},
	{ 21, "LOAD_GUEST_IA32_LBR_CTL"},
	{ 22, "LOAD_PKRS"}
};

struct capability_info procbasedctl3[4] = {
        {0,"LOADIWKEY_EXITING"},
        {1,"ENABLE_HLAT"},
        {2,"EPT_PAGING-WRITE_CONTROL"},
        {3,"GUEST_PAGING_VERIFICATIION"}
};
/*
 * report_capability
 *
...
detect_vmx_features(void)
{
	...
	pr_info("Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedctl, 22, lo, hi);
	...
	pr_info("Procbased Secondary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedctl2, 27, lo, hi);
	...
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exitctl, 16, lo, hi);
	...
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entryctl, 13, lo, hi);
	...
	pr_info("Tertiary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedctl3, 4, lo, hi);
